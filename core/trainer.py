...
class Trainer(object):
    ...
    def __init__(self, config):
        ...
        self.result_path, self.log_path, self.checkpoints_path, self.viz_path = self._init_files(
            config
        )
    ...
    def _train(self, epoch_idx):
        ...
        meter.reset()
        episode_size = (
            1 if self.model_type == ModelType.PRETRAIN else self.config["episode_size"]
        )

        end = time()
        for batch_idx, batch in enumerate(self.train_loader):
            self.writer.set_step(
                epoch_idx * len(self.train_loader) + batch_idx * episode_size
            )
            ...
            if (
                batch_idx != 0 and batch_idx % self.config["log_interval"] == 0
    ...
    def _init_files(self, config):
        ...
        result_dir = (
            symlink_dir + "-{}".format(get_local_time())
                ...
                for parallel_part in parallel_list:
                    if hasattr(model, parallel_part):
                        setattr(
                            ...
                            parallel_part,
                            nn.DataParallel(
                                getattr(model, parallel_part), device_ids=self.list_ids
                            ),
                        )
