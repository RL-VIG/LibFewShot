...
class RelationNet(MetricModel):
    def __init__(self, train_way, train_shot, train_query, emb_func, device, feat_dim=64,
                 feat_height=3, feat_width=3):
        super(RelationNet, self).__init__(train_way, train_shot, train_query, emb_func,
                                          device)
    ...
    def set_forward(self, batch, ):
        ...
        output = self.relation_layer(relation_pair).view(-1, self.train_way)
    ...
    def _calc_pairs(self, query_feat, support_feat):
        ...
        :param query_feat: (task_num, train_query * train_way, feat_dim, feat_width, feat_height)
        :param support_feat: (task_num, support_num * train_way, feat_dim, feat_width, feat_height)
        :return: train_query * train_way * train_way, feat_dim, feat_width, feat_height
        """
        t, _, c, h, w = query_feat.size()
        # t, w, wq, c, h, w -> t, wq, w, c, h, w
        query_feat = query_feat.unsqueeze(1).repeat(1, self.train_way, 1, 1, 1, 1)
        ...
        support_feat = torch.sum(support_feat, dim=(2,)).unsqueeze(1) \
            .repeat(1, self.train_way * self.train_query, 1, 1, 1, 1)
