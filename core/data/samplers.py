...
class CategoriesSampler(Sampler):
    ...
    def __init__(self, label_list, label_num, episode_size, episode_num, train_way,
                 image_num):
    ...
    def __iter__(self):
        ...
        for i_batch in range(self.episode_num):
            classes = torch.randperm(len(self.idx_list))[:self.train_way]
            for c in classes:
                idxes = self.idx_list[c.item()]
                pos = torch.randperm(idxes.size(0))[:self.image_num]
                batch.append(idxes[pos])
            if len(batch) == self.episode_size * self.train_way:
